<script>
  ...
  const handleSignUp = async(e) => {
    ...
    try{
      if(!email_address || !password){
        form_error = "Please fill out all the details!"
        setTimeout(()=>{
          form_error = '';
        },2000)
        return;
      ...
      const user = await authService.signUp({email : email_address, password})
  ...
  const handleGoogleSignUp = async() =>{
    try{
      ...
      console.log("Error Register Page :: handleGoogleSignUp :: ",e.message)
...
<div class="min-h-screen bg-gray-100 text-gray-900 flex justify-center">
  ...
  >
    ...
    >
      <h3
        class="text-xl sm:text-2xl font-semibold mb-6 flex flex-col sm:flex-row items-center"
      >
        <div
          class="bg-green-200 p-2 rounded-full flex items-center mb-4 sm:mb-0 sm:mr-2"
        >
          <svg
      ...
      <p>
        I recently released Treact, a <span class="font-bold">free</span> TailwindCSS
